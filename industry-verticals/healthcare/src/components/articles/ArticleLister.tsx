...
interface ArticleListerProps extends ComponentProps {
...
export const Default = (props: ArticleListerProps) => {
  const { t } = useI18n();
            ...
            {Object.values(categoryCounts).map((category) => (
              <li key={category.name} className="flex justify-between">
                <button
                  ...
                  className={`flex-1 text-left ${selectedCategory === category.name ? 'text-accent font-bold' : ''
                    }`}
                >
                  {category.name}
                </button>
